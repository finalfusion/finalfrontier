...
use finalfrontier::similarity::Analogy;
use finalfrontier::{Model, ReadModelBinary, Vocab};
use rayon::prelude::*;
...
fn main() {
    ...
    process_analogies(&model, &instances);
}
...
impl<'a> Eval<'a> {
    ...
    fn eval_analogy(&self, instance: &Instance) {
        ...
        if self.model.vocab().idx(&instance.answer).is_none() {
...
fn process_analogies(model: &Model, instances: &[Instance]) {
